...
import '../utils/rect_extension.dart';
...
class Kakao {
  ...
  List<Block> mergeTextBlocksKakao(List<Block> textBlocks) {
    ...
    List<Block> sortedByTopBlocks = textBlocks.where((block) => block.blockFrame.left <= nameLeft).toList();
    sortedByTopBlocks.sort((a, b) => a.blockFrame.top.compareTo(b.blockFrame.top));

    List<Block> sortedByLeftBarcodeBelow = textBlocks.where((block) => block.blockFrame.bottom >= nameBottom).toList();
    sortedByLeftBarcodeBelow.sort((a, b) => a.blockFrame.left.compareTo(b.blockFrame.left));
    ...
    for (int i = 1; i < sortedByTopBlocks.length; i++) {
      ...
      if ((block.blockFrame.left - name.blockFrame.left).abs() <= marginLeft &&
          ...
          (block.blockFrame.top - name.blockFrame.bottom).abs() <= marginBetweenTopBottom) {
    ...
    for (Block cur in sortedByLeftBarcodeBelow) {
      for (Block block in sortedByLeftBarcodeBelow) {
        ...
        if (!StringIncluded.check(pattern, block.blockText)) {
          if ((block.blockFrame.top - cur.blockFrame.top).abs() <= marginSameTop) {
