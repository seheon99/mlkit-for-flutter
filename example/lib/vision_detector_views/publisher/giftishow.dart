...
class Giftishow {
  ...
  List<Block> cropGiftishow(List<Block> textBlocks) {
    ...
    var sortedByTopBlocks = List<Block>.from(textBlocks)
      ..sort((block1, block2) => block2.blockFrame.top.compareTo(block1.blockFrame.top));

    for (var block in sortedByTopBlocks) {
      String text = block.blockText;

      if (StringIncluded.check('기프티쇼', text) || StringIncluded.check('giftishow', text)) {
  ...
  List<Block> mergeTextBlocksGiftishow(List<Block> textBlocks) {
    var sortedByTopBlocks = List<Block>.from(textBlocks)
      ..sort((block1, block2) => block1.blockFrame.left.compareTo(block2.blockFrame.left));
